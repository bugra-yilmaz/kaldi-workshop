...
ls -1 $waves_dir"/train" > data/local/waves.train
ls -1 $waves_dir"/test" > data/local/waves.test

cd data/local

../../local/create_yesno_wav_scp.pl ${waves_dir}"/test" waves.test > ${test_base_name}_wav.scp

../../local/create_yesno_wav_scp.pl ${waves_dir}"/train" waves.train > ${train_base_name}_wav.scp
